...
void TlvParserWindow::initUi()
{
    ...
    if (parsedField) {
        ...
        parsedField->setAnimated(true);
        connect(parsedField, SIGNAL(itemClicked(QTreeWidgetItem *, int)), this, SLOT(tagClicked(QTreeWidgetItem *, int)));
        connect(parsedField, SIGNAL(itemDoubleClicked(QTreeWidgetItem *, int)), this, SLOT(tagDoubleClicked(QTreeWidgetItem *, int)));
    }
    if (parseButton) {
    ...
    if (parseField) {
        parseField->setValidator(new QRegularExpressionValidator(re, this));
        connect(parseField, SIGNAL(textChanged(const QString &)), this, SLOT(enableParseButton(const QString &)));
    }
...
void TlvParserWindow::parse()
{
    QString tlvData;
    unsigned int status = 1;

    if (parseField && parsedField) {
        tlvData = parseField->text().replace(" ", "");
        TlvParser parser(QByteArray::fromHex(tlvData.toUtf8()));
        parsedField->clear();
        status = parser.parseToTree(parsedField);
    }
    if (status) {
        if (parseField) {
            parsedField->clear();
...
void TlvParserWindow::enableParseButton(const QString& text)
{
    if (parseButton && parseField) {
        parseButton->setEnabled(parseField->hasAcceptableInput());
        if (parseField->text().replace(" ", "").length() % 2) {
            parseButton->setEnabled(false);
        }
    }
}

void TlvParserWindow::tagClicked(QTreeWidgetItem *item, int column)
{
    QString text;
    QString tagInfoString;
    TlvInfo tagInfo;
    ...
    if (!text.contains('\n')) {
        tag = text.section(' ', 0, 0).toUInt(&ok, 16);
        if (!tlvInfoGetFromFile(tag, &tagInfo)) {
            tagInfoString = buildTagInfo(&tagInfo);
            text += "\n" + tagInfoString;
        }
    } else {
        text = text.section('\n', 0, 0);
...
void TlvParserWindow::tagDoubleClicked(QTreeWidgetItem *item, int column)
{
    if (!item) {
        return;
    }
    if (item->text(column).contains('\n')) {
        item->setText(column, item->text(column).section('\n', 0, 0));
    }
}

QString TlvParserWindow::buildTagInfo(TlvInfo* tagInfo)
{
    if (!tagInfo) {
        return "";
    }
